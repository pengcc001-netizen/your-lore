...
interface Answer { questionId: number; optionIndex: number; stats: { strength?: number; wisdom?: number; charisma?: number; cunning?: number; resilience?: number; magic?: number } }

const STAT_KEYS = ['strength', 'wisdom', 'charisma', 'cunning', 'resilience', 'magic'] as const

function getArchetype(totalStats: Record<string, number>): typeof archetypes[0] {
  const dominant = STAT_KEYS.reduce((max, key) => totalStats[key] > totalStats[max] ? key : max, 'strength')
  const archetypeMap: Record<string, string> = { strength: 'the-warlord', wisdom: 'the-mentor', charisma: 'the-sovereign', cunning: 'the-trickster', resilience: 'the-exile', magic: 'the-oracle' }
  const slug = archetypeMap[dominant]
  return archetypes.find(a => a.slug === slug) || archetypes[0]
}
...
export default function Home() {
  ...
  const handleAnswer = (optionIndex: number) => {
    ...
    const newAnswers = [...answers, { questionId: q.id, optionIndex, stats: opt.stats }]
    ...
    return (
      <div className="fade-in">
        ...
        <section style={{ textAlign: 'center', padding: '40px 0 60px' }}>
          <div className="eyebrow" style={{ marginBottom: 16 }}>Character Lore Generator · 2026</div>
          ...
          <p style={{ fontSize: 'clamp(1rem, 2vw, 1.25rem)', color: 'var(--text-soft)', maxWidth: 560, margin: '0 auto 40px', lineHeight: 1.7, fontFamily: 'var(--font-body)' }}>
            Answer 12 questions and uncover your epic backstory, abilities, fatal flaw, signature item, and quest. Which of {archetypes.length} archetypes are you?
          </p>
          <button onClick={() => setPhase('quiz')} className="btn btn-gold" style={{ padding: '16px 48px', fontSize: 14 }}>Begin Your Journey →</button>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 16, marginBottom: 60 }}>
          {[{ label: 'Character Archetypes', value: `${archetypes.length}` }, { label: 'Unique Abilities', value: '40+' }, { label: 'Stat Categories', value: '6' }, { label: 'Questions', value: '12' }].map((stat, i) => (
            <div key={i} className="gold-card" style={{ padding: '24px 20px', textAlign: 'center' }}>
              <div className="mono" style={{ fontSize: '1.75rem', fontWeight: 700, color: 'var(--gold)', marginBottom: 4 }}>{stat.value}</div>
        ...
        <section>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 24 }}>
            <div><div className="eyebrow" style={{ marginBottom: 4 }}>Featured</div><h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 'clamp(1.5rem, 3vw, 2rem)' }}>The Archetypes</h2></div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
            {archetypes.slice(0, 4).map((a) => (
              <Link key={a.slug} to={`/archetypes/${a.slug}`} style={{ textDecoration: 'none' }}>
                <div className="gold-card" style={{ padding: '24px', height: '100%' }}>
                  <div style={{ fontSize: '2rem', marginBottom: 8 }}>{a.emoji}</div>
                  <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '1.125rem', fontWeight: 700, color: 'var(--text)', marginBottom: 4 }}>{a.name}</h3>
                  <p style={{ fontSize: 13, color: 'var(--gold)', fontStyle: 'italic', fontFamily: 'var(--font-body)' }}>"{a.tagline}"</p>
                </div>
              </Link>
            ))}
          </div>
  ...
  if (phase === 'quiz') {
    ...
    return (
      <div className="fade-in" style={{ maxWidth: 720, margin: '0 auto' }}>
        <Helmet><title>Character Quiz - Question {currentQ + 1} of {quizQuestions.length} | Your Lore</title></Helmet>
        <div style={{ marginBottom: 40 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
            ...
            <span className="mono" style={{ fontSize: 12, color: 'var(--gold)' }}>{Math.round(progress)}%</span>
          </div>
          <div style={{ height: 4, background: 'var(--border)', borderRadius: 2, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${progress}%`, background: 'linear-gradient(90deg, var(--gold-dark), var(--gold-bright))', borderRadius: 2, transition: 'width 0.4s ease' }} />
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {q.options.map((opt, i) => (
            <button key={i} onClick={() => handleAnswer(i)} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '20px 24px', textAlign: 'left', cursor: 'pointer', transition: 'all 0.2s ease', color: 'var(--text)', fontSize: 15, fontFamily: 'var(--font-body)', lineHeight: 1.5 }}
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--gold)'; e.currentTarget.style.background = 'var(--gold-soft)' }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.background = 'var(--bg-card)' }}>
              <span style={{ color: 'var(--gold)', fontFamily: 'var(--font-mono)', fontWeight: 600, marginRight: 12 }}>{String.fromCharCode(65 + i)}</span>{opt.text}
            </button>
          ))}
        </div>
  ...
  if (phase === 'result') {
    const totalStats: Record<string, number> = { strength: 0, wisdom: 0, charisma: 0, cunning: 0, resilience: 0, magic: 0 }
    answers.forEach(a => { Object.entries(a.stats).forEach(([k, v]) => { totalStats[k] = (totalStats[k] || 0) + (v || 0) }) })
    const archetype = getArchetype(totalStats)
    const shareUrl = typeof window !== 'undefined' ? window.location.href : 'https://lore.csskey.com'
    const maxStat = Math.max(...Object.values(totalStats))

    return (
      <div className="fade-in" style={{ maxWidth: 680, margin: '0 auto' }}>
        <Helmet><title>Your Lore: {archetype.name} | Your Lore</title><meta name="description" content={`My character lore: ${archetype.name}. ${archetype.tagline} Discover yours!`} /><meta name="robots" content="noindex,nofollow" /></Helmet>
        <div className="gold-card gold-card-ornate" style={{ padding: '40px 32px', marginBottom: 32 }}>
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <div className="mono" style={{ fontSize: 11, letterSpacing: '0.3em', color: 'var(--gold)', marginBottom: 8 }}>CHARACTER SHEET</div>
          ...
          <div style={{ marginBottom: 32 }}>
            <div className="mono" style={{ fontSize: 11, letterSpacing: '0.2em', color: 'var(--text-faint)', marginBottom: 12 }}>ATTRIBUTES</div>
            {STAT_KEYS.map(key => (
              <div key={key} style={{ marginBottom: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span className="mono" style={{ fontSize: 12, color: 'var(--text-soft)', textTransform: 'capitalize' }}>{key}</span>
                  <span className="mono" style={{ fontSize: 12, color: 'var(--gold)', fontWeight: 600 }}>{totalStats[key]}</span>
                </div>
                <div style={{ height: 6, background: 'var(--border)', borderRadius: 3, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${(totalStats[key] / maxStat) * 100}%`, background: 'linear-gradient(90deg, var(--gold-dark), var(--gold))', borderRadius: 3, transition: 'width 0.5s ease' }} />
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Signature Item */}
          <div style={{ marginBottom: 24, paddingTop: 24, borderTop: '1px solid var(--border)' }}>
            <div className="eyebrow" style={{ marginBottom: 8 }}>Signature Item</div>
          ...
          <div style={{ paddingTop: 24, borderTop: '1px solid var(--border)' }}>
            <div className="eyebrow" style={{ marginBottom: 8 }}>Your Quest</div>
            <p style={{ fontSize: 14, lineHeight: 1.7, color: 'var(--text)', fontFamily: 'var(--font-body)' }}>{archetype.quest}</p>
          </div>
        </div>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <div className="section-label" style={{ marginBottom: 16 }}>Share Your Lore</div>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(`My character lore is ${archetype.name}! Discover yours:`)}&url=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noopener" className="btn btn-outline" style={{ fontSize: 12 }}>Share on X</a>
        ...
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <button onClick={resetQuiz} className="btn btn-ghost" style={{ fontSize: 13 }}>← Retake the Quiz</button>
        </div>
        <div style={{ textAlign: 'center' }}>
          <div className="section-label" style={{ marginBottom: 16 }}>Explore More</div>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link to="/archetypes" className="btn btn-outline" style={{ fontSize: 12 }}>All Archetypes</Link>
            <Link to="/blog" className="btn btn-outline" style={{ fontSize: 12 }}>Read the Lorekeeper</Link>
          </div>
        </div>
      </div>
    )
