...
export default function ResultPage() {
  ...
  if (!decoded) {
    return (
      <div style={{ textAlign: 'center', padding: '60px 0' }}>
        <Helmet><meta name="robots" content="noindex,nofollow" /><title>404 - Not Found | Your Lore</title></Helmet>
        <p style={{ color: 'var(--text-soft)', fontFamily: 'var(--font-mono)' }}>Invalid lore link.</p>
  ...
  decoded.forEach((optionIndex, i) => {
    const opt = quizQuestions[i].options[optionIndex]
    Object.entries(opt.stats).forEach(([k, v]) => { totalStats[k] = (totalStats[k] || 0) + (v || 0) })
  })
          ...
          {STAT_KEYS.map(key => (
            <div key={key} style={{ marginBottom: 12 }}>
              ...
              <div style={{ height: 6, background: 'var(--border)', borderRadius: 3, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${(totalStats[key] / maxStat) * 100}%`, background: 'linear-gradient(90deg, var(--gold-dark), var(--gold))', borderRadius: 3 }} />
              </div>
            </div>
          ))}
        </div>
